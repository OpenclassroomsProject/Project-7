...
import { faShare, faEllipsisVertical, faThumbsUp as faThumbsUpLiked, faTrashCan, faPencil, faFlag } from '@fortawesome/free-solid-svg-icons';
...
// @ts-ignore
function PostTemplate({ title, textArea, _id, createBy, createByPseudo, imagesUrl, date, likes, usersLiked }) {
    ...
    return (
        <>
            <section className={`relative bg-white flex flex-col mt-2  text-[#5d5c5c] `} id={_id} key={_id}>
                <div>
                    <div className=' relative pt-3 pl-4 pr-4 text-base'>
                        <div className='flex items-center '>
                            ...
                            <div className='mr-auto'>
                                <Link to={'/profil/' + createBy}>
                                    <div className=' text-sm font-serif font-semibold text-black'> {createByPseudo} </div>
                                </Link>
                        ...
                        {btnMore ? (
                            <div
                                className={` w-fit absolute leading-5  right-0  ${openDescritpion ? 'bottom-0' : 'bottom-[3%]'}
                        ...
                        <Link to={clickEdit ? '/' : '/post/' + _id} className='  '>
                            <p
                                ref={ref_description}
                                className={`relative overflow-hidden text-left text-sm mt-4 mb-2  ${!openDescritpion && 'max-h-16  '}`}
                ...
                {/* =======================================================  BUTTON  OPTION  ======================================================= */}
                <button className='absolute right-2 top-2 text-[#aaa] cursor-pointer p-2 pl-4 pr-4  ' onClick={() => setClickOption(!clickOption)}>
                    ...
                    <div className='absolute bg-white  z-10  right-0 top-[7%] flex   flex-col shadow-2xl items-start  font-semibold pt-2 pb-2 border-[1px] '>
                        {admin ? (
                            <>
                                ...
                                <button className='pr-8 pl-4 leading-[3rem] mr-auto' onClick={handleClickDeletePost}>
                ...
                {imagesUrl[0] ? <img className='border-b-[1px]' src={server + 'images/' + createBy + '/' + imagesUrl} alt='post'></img> : false}
                ...
                <div className='flex justify-between items-center pl-4 pr-4 h-8 border-w-1 border-#f2f2f2 border-b-[1px]  text-xs text-[#aaa] '>
                    <div className='flex'>
                        <div className=' bg-blue-500 rounded-full flex h-fit mr-1 mt-auto mb-auto'>
                            <FontAwesomeIcon icon={faThumbsUpLiked} className='text-blue-100 p-1 text-[0.5rem] ' />
                        </div>
                        {allLikes}
                    </div>
                ...
                {clickEdit && admin && (
                    // @ts-ignore
                    <EditPost contentToEdit={textArea} />
                )}
            </section>
        </>
    );
