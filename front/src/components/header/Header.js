// import logo from './../public/icon-left-font.svg';
// import userOnMobile from './_detectMobile';
import {Link} from 'react-router-dom'
import RightPanel from './_RigthPanel';
import AuthForm from './_AuthForm';

import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faGear, faHouse, faBell, faPlus } from '@fortawesome/free-solid-svg-icons';
import { faUser } from '@fortawesome/free-regular-svg-icons';

import React, { useState } from 'react';

<script src="https://kit.fontawesome.com/11d72bfdc7.js" crossorigin="anonymous"></script>;




const Header= function ({ userConnected }){
    const userIsConnected = userConnected[0];
    const setUserConnected = userConnected[1];

    const NavDynamique = ({ mobile }) => {
        const NavConnected = () => {
            ...
            const cssButton = ' m-auto cursor-pointer w-10  h-10 border-[2px] flex justify-center items-center  rounded-full overflow-hidden ';
            const style = {
                button: cssButton,
                buttonPush: cssButton + 'border-primary-red text-secondary-pink shadow-xl border-[4.01px]',
                // hover:[shadow-xl, p-1.5, border-[4px]]
                buttonHover: 'hover:border-[3px] hover:shadow-xl  hover:border-secondary-pink hover:text-secondary-pink ',
                iconButton: 'p-1',
            };
            return (
                <>
                    ...
                    <Link to="/" className={style.button} >
                            <FontAwesomeIcon className={style.iconButton} icon={faHouse} height="100%" />
                    </Link>
                    {/* <div className={style.button}>
                        <FontAwesomeIcon icon={faPlus} height="100%" />
                    </div> */}

                    <div className={`${ClickBell ? style.buttonPush : style.button + style.buttonHover} mr-2 ml-2`} onClick={() => setClickBell( !ClickBell )}>
                        <FontAwesomeIcon className={style.iconButton} icon={faBell} height="100%" />
                    </div>

                    <button
                        className={`w-24 flex items-center justify-between  ${ClickProfil ? style.buttonPush : style.button + style.buttonHover} `}
                        onClick={() => setClickProfil(!ClickProfil )}
                    >
                        <div className={`h-full w-[40%] ${style.iconButton}`}>
                            <img className="rounded-full h-full" src="/code.jpg" height={"100%"} alt="user profil"/>
                        </div>
                        <p className="mr-auto">Profil</p>
                    </button>

                    {ClickProfil ? (
                        <div className="absolute bg-tertiary-black w-60 h-28 top-16 right-10 rounded p-2 pl-3 pr-3 pt-3 flex flex-col shadow-2xl z-10">
                            <div className="w-full  h-full grid grid-cols-3">
                                {/* <div className="flex"> */}

                                    <Link to={'/createPost'} className={`${style.button} ${style.buttonHover}`}>
                                    ...
                                    <Link to={'/profil/setting'} className={`${style.button} ${style.buttonHover}`}>
                                        <FontAwesomeIcon icon={faGear} />
                                    </Link>

                                {/* </div> */}
                            </div>
                            <button className="bg-white text-tertiary-black h-10  rounded-sm" onClick={logOut}>
                                Déconexion
                            </button>
                        </div>
                    ) : (
        ...
        const NavVisitors = () => {
            return (
                <>
                        ...
                        <Link to="/login" className="login" action="Login" onClick={toggleForm}>
                            Connexion
                        </Link>


                        <Link to="/signIn" className="signin" action="SignIn" onClick={toggleForm}>
                            Inscription
                        </Link>
        ...
        if (userIsConnected) return <NavConnected></NavConnected>;
        return <NavVisitors></NavVisitors>;
    };
    

    //========== EventListener on mobile humburger icon ==================
    ...
    const [logPanel, setLogPanel] = useForm(null);

    function toggleForm(e) {
        e.preventDefault();

        // if (userOnMobile() && ClickHamburger) {
        //     toggleMenuHumberger();
        // }

        const form = e.target.getAttribute('action');

        if (logPanel && logPanel.formType === form) return closePanel();
        // test();
        // console.log(document.getElementsByClassName('rightPanel')[0]);
        // document.getElementsByClassName('rightPanel')[0].removeEventListener('click', close);
        return setLogPanel({ formType: form });
    }

    const closePanel = () => {
        if (!logPanel) return false;
        return setLogPanel(false);
    };
    //====================  End EventListener Form ========================

    const class_buttonMobile = 'cursor-pointer w-10 h-10  flex justify-center items-center';
    // const class_buttonMobile = '';


    return (
            <header className="flex justify-center h-14 shadow-xl pl-4 w-full  bg-tertiary-black     text-white sm:">
                {userIsConnected ? (
                    <div className={`absolute left-2 h-14 flex items-center justify-start  sm:hidden`}>
                        <FontAwesomeIcon icon={faBell} height="50%" />
                    </div>
                ) : (
                    false
                )}
                {/* ==============================  Logo  ============================== */}
                    ...
                    <Link to="/" className="h-full flex items-center  sm:mr-auto ">
                        <svg className="cursor-pointer" data-v-5f19e91b="" xmlns="http://www.w3.org/2000/svg" height="80%" viewBox="0 0 485 78">
                            <g
                                data-v-5f19e91b=""
                ...
                <nav className={' hidden items-center sm:flex  p-2  pr-0 '}>
                    <NavDynamique></NavDynamique>
                </nav>

                {/* =================== AuthPanel ===================== */}
                {logPanel != null ? (
                    <RightPanel visible={logPanel ? true : false} unmount={setLogPanel}>
                        <AuthForm
                            type={logPanel.formType ? logPanel.formType : false}
                            setUserConnected={(_id = false) => {
                                setLogPanel(false);
                                setUserConnected(_id);
                            }}
                        ></AuthForm>
                    </RightPanel>
                ) : (
                ...
                {/* ============================ EndNav Desktop ============================= */}
                {/* ============================== Nav mobile  ============================== */}
                <nav className="absolute bottom-0 left-0 bg-tertiary-black  w-full h-12 flex items-center justify-around sm:hidden  ">

                        <Link to="/" className={`${class_buttonMobile}`}>
                            <FontAwesomeIcon icon={faHouse} height="60%" />
                        </Link>


                        <Link to={'/createPost'} className={`${class_buttonMobile} rounded-full bg-white`}>
                            <FontAwesomeIcon className="text-tertiary-black h-8" icon={faPlus} height="90%" />
                        </Link>

                    <Link to={!userIsConnected ? '/login' : '/profil?id=' + userIsConnected}>
...
// function detectPath(searchPath) {
//     if (window.location.pathname === searchPath) return true;
//     return false;
// }
