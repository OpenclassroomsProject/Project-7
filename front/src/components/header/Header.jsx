...
import { faGear, faHouseChimney, faBell, faPlus } from '@fortawesome/free-solid-svg-icons';
...
const Header = function () {
    ...
    //     if (context.userData) {
    //         setUserConnected(context.userData);
    //     }
    // }, [context]);
    // useEffect(() => {
    //     console.log(theme);
    ...
    const NavDynamique = () => {
            ...
            return (
                <>
                    ...
                    <Link to='/' className={style.button}>
                        <FontAwesomeIcon className={style.iconButton} icon={faHouseChimney} height='100%' />
                    </Link>
                    {/* <div className={style.button}>
                        <FontAwesomeIcon icon={faPlus} height="100%" />
                    </div> */}
                    <div
                        className={`${ClickBell ? style.buttonPush : style.button + style.buttonHover} mr-2 ml-2`}
        ...
        // @ts-ignore
        if (userContext.userData) return <NavConnected></NavConnected>;
    ...
    const getALlStyle = () => {
        let allStyle = '';
        if (!themeContext.header) return false;
        Object.entries(themeContext.header).forEach((style) => {
            if (style[0] !== 'fontColor') {
                allStyle += ' ' + style[1];
            }
        });
    ...
    return (
        ...
        (
            <header className={`flex justify-center h-12 shadow-xl pl-4 w-full ${themeContext.header.fontColor}   ${getALlStyle()}`}>
                {/* ==============================  Logo  ============================== */}
                ...
                >
                    ...
                    <Link
                        to={
                            // @ts-ignore
                            !userContext.userData._currentValue
                                ? '/login'
                                : '/profil?id=' +
                                  // @ts-ignore
                                  userContext.userData._currentValue.id
                        }
                        // @ts-ignore
                    ...
                    <Link
                        // @ts-ignore
                        to={!userContext.userData._currentValue ? '/login' : '/notifs'}
                        // @ts-ignore
                        className={`${class_buttonMobile} ${pageActive.notifs ? 'text-black' : ''}  `}
                        onClick={() => {
                            // @ts-ignore
                            setPageActive({ notifs: true });
                        }}
                    >
                    ...
                    <Link
                        // @ts-ignore
                        to={!userContext.userData._currentValue ? '/login' : './profil/setting'}
                        // @ts-ignore
                    ...
                    >
                        <FontAwesomeIcon icon={faGear} height='50%' />
                    </Link>
                </nav>
                {/* ============================== End nav mobile  ============================== */}
            </header>
